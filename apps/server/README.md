...
### An idle fan-out socket is pinged, not left silent (since C19)

The bound above is about a subscriber receiving too much. The opposite case is a subscriber receiving nothing, and it is the ordinary one: a dashboard watching a device that has stopped sending gets no frames and no alerts, for as long as the device stays quiet. Device disconnect is the first failure mode in [docs/ARCHITECTURE.md](../../docs/ARCHITECTURE.md), so this is a state the system is designed to reach — and until C19 the server sent nothing on that socket either, which every intermediary between the two ends reads as a dead connection. An AWS load balancer's idle timeout and nginx's `proxy_read_timeout` both default to 60 seconds.

`STREAM_HEARTBEAT_MS` (25 s, [src/stream.ts](src/stream.ts)) is the server's stated maximum silence on a `/stream` socket: a WebSocket ping control frame, so two fit inside those defaults and one lost ping closes nothing. It is a control frame rather than a fourth message type because `streamMessageSchema` is a strict union that [packages/protocol](../../packages/protocol) asserts rejects `{type:"heartbeat"}` — an application-layer keepalive would have to be learned by apps/web and apps/ios on the day it shipped, while a ping is answered by the browser, by `ws` and by `URLSessionWebSocketTask` without any of them being told. `/ingest` has no heartbeat and needs none: a device is either sending or genuinely gone.

The failure mode this introduces is the timer that outlives its socket, and it is why the interval is deliberately not `unref()`d. An unref'd leak is invisible; a ref'd one holds the event loop open, so `SIGTERM` never completes and the orchestrator SIGKILLs — the C18 failure this repository has already paid for once, and one with a test. [src/stream.heartbeat.test.ts](src/stream.heartbeat.test.ts) spawns a real process, attaches a subscriber, waits for a ping, disconnects it, and asserts the server still exits 0; removing the `clearInterval` hangs that test instead of passing it quietly. The quiet failure it does not catch is stated rather than implied: adding `.unref()` alongside the leak makes the whole thing green again, which is the measurement behind the choice ([docs/ai/mutation-log.md](../../docs/ai/mutation-log.md)).
...
| [src/stream.heartbeat.test.ts](src/stream.heartbeat.test.ts)   | the fan-out keepalive: an idle subscriber is pinged more than once, the ping never reaches an application message handler, a real spawned server still exits 0 after that subscriber leaves, and the `STREAM_HEARTBEAT_MS` bounds                                 |
