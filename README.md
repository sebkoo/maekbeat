...
| 6 — Infra & operations   | OTel tracing (C18), Docker + compose, k6 load rig, AWS CDK stack                                                                              | ⬜     | [C18 tracing](https://github.com/sebkoo/maekbeat/commit/3260723) · [C19 container, load rig, AWS stack](https://github.com/sebkoo/maekbeat/compare/3260723...11c9adf) · CI image build, registry publish and dashboards-as-code still to come                                                                                                                                                           |
...
Updated in the same commit as every scope change. **One chip per roadmap row, never one per commit**: a commit link where the row is a single commit, a compare link across its range where the row holds several ([docs/ROADMAP.md](docs/ROADMAP.md) defines what a row is). Commit-level detail belongs in the roadmap, which lists every commit under its row. A commit cannot link itself, so the newest row's chip is backfilled by the next commit that touches the board — the one-commit lag is by design, and `scripts/check-commit-links.sh` allows it exactly once.
