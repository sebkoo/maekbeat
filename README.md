...
| Scaling model and load evidence             | target architecture + measured k6 numbers on one laptop, labelled with it ([infra/README.md](infra/README.md))                         | C4 ✅ · C19 ✅                     |
...
| Alarming on the absence of data             | a sweep over last-seen stamps, threshold derived from the gateway's reconnect deadlines (apps/server/src/silence.ts)                   | C20a ✅                            |
| Alarm fatigue and acknowledgement           | episode timeline + append-only decision log (apps/server/src/acks.ts)                                                                  | C12 ✅ · C16 ✅ · C20a ✅ · C21 ✅ |
...
| Instrumentation before observability        | one trace per frame (apps/server/src/tracing.shape.test.ts); no dashboard, and the reason ([docs/DECISIONS.md](docs/DECISIONS.md) #32) | C18 ✅ · C19 ✅                    |
| Proving a container is the commit it claims | image label and /healthz revision, both asserted against git ([infra](infra))                                                          | C19 ✅                             |
| Hazard rows a build can check               | [docs/regulatory](docs/regulatory) — each row cites its test, `scripts/check-hazard-tests.sh` fails when one stops resolving           | C20 ✅ · C20a ✅ · C21 ✅          |
| Health-data security posture                | [SECURITY.md](SECURITY.md) (today) · docs/security/threat-model.md                                                                     | C22 ✅                             |
| iOS background execution + BLE lifecycle    | five-phase machine + background notes, [apps/ios](apps/ios) README                                                                     | C15 ✅                             |
| Tooling choices and trade-offs              | [docs/DECISIONS.md](docs/DECISIONS.md) (32 entries today)                                                                              | C0 ✅                              |
...
```text
apps/        server (WS ingest · ring buffer · alert engine · REST reads · fan-out · decisions · tracing · build identity · silence alarm — C5–C20a)
             web (tokens · live chart · timeline · acknowledgement · WCAG 2.2 AA · e2e smoke — C10–C20a)
...
infra/       server + web images, compose stack, k6 load rig, image and stack proofs — C19;
             infra/cdk — the AWS stack, synthesized and asserted, never deployed — C19
...
| 3 — Server               | Fastify, WS ingest, alert engine, tests, coverage gate                                                                                        | ✅     | [C5 skeleton](https://github.com/sebkoo/maekbeat/commit/d352705) · [C6 ingest](https://github.com/sebkoo/maekbeat/commit/0170638) · [C7 alerts](https://github.com/sebkoo/maekbeat/commit/2a1d563) · [C8 tests](https://github.com/sebkoo/maekbeat/commit/2356a62) · [C9 gate](https://github.com/sebkoo/maekbeat/commit/eba4e44) · [C12a retention](https://github.com/sebkoo/maekbeat/commit/5ac4510)            |
| 4 — Web                  | React scaffold, live chart, timeline + ack, tests                                                                                             | ✅     | [C10 tokens](https://github.com/sebkoo/maekbeat/commit/6e9c81c) · [C11 live chart](https://github.com/sebkoo/maekbeat/commit/8dfe023) · [C12 timeline + ack + WCAG](https://github.com/sebkoo/maekbeat/commit/66e30df) · [C13 smoke](https://github.com/sebkoo/maekbeat/commit/18aa597)                                                                                                                            |
| 5 — iOS                  | SwiftUI scaffold + simulator transport, CoreBluetooth central + gateway, notifications, XCTest                                                | ✅     | [C14 scaffold](https://github.com/sebkoo/maekbeat/commit/ec08ac5) · [C15 BLE + gateway](https://github.com/sebkoo/maekbeat/commit/ad96c2f) · [C16 notifications](https://github.com/sebkoo/maekbeat/commit/dfb105a) · [C17 seams + ratchet](https://github.com/sebkoo/maekbeat/commit/cbf37d2)                                                                                                                     |
| 6 — Infra & operations   | OTel tracing (C18), Docker + compose, k6 load rig, AWS CDK stack                                                                              | ✅     | [C18 tracing](https://github.com/sebkoo/maekbeat/commit/3260723) · [C19 container, load rig, AWS stack](https://github.com/sebkoo/maekbeat/compare/3260723...11c9adf) · [C19 delivery pipeline](https://github.com/sebkoo/maekbeat/compare/bccd59e...9b93a4f)                                                                                                                                                      |
| 7 — Depth                | intended use, hazard analysis, risk register, silence alarm, threat model, SBOM                                                               | ✅     | [C20 hazard analysis + its guard](https://github.com/sebkoo/maekbeat/commit/4b76c8b) · [C20a the alarm that analysis found missing](https://github.com/sebkoo/maekbeat/commit/bccd59e) · [C21 lifecycle map, SOUP inventory, risk register](https://github.com/sebkoo/maekbeat/compare/7e80bb5...eeecfcf) · [C22 SBOM, data-flow, STRIDE, audit log](https://github.com/sebkoo/maekbeat/compare/02c31f1...f117d2d) |
| 8 — Release              | v0.1.0                                                                                                                                        | ⬜     | C23                                                                                                                                                                                                                                                                                                                                                                                                                |

Updated in the same commit as every scope change. **One chip per contiguous span of a roadmap row, never one per commit**: a commit link where the row is a single commit, a compare link across its range where the row holds several ([docs/ROADMAP.md](docs/ROADMAP.md) defines what a row is). C19 is the only row with two, because it was left open while C20 and C20a shipped and its commits therefore sit on both sides of them; `scripts/check-commit-links.sh` checks coverage against the union of a row's chips and still refuses any single one that reaches across another row's commit. Commit-level detail belongs in the roadmap, which lists every commit under its row. A commit cannot link itself, so the newest row's chip is backfilled by the next commit that touches the board — the one-commit lag is by design, and `scripts/check-commit-links.sh` allows it exactly once.
...
| Web     | React 19, Vite, TypeScript                                                                              | tokens, live chart, timeline, acknowledgement, WCAG 2.2 AA, e2e smoke ([apps/web](apps/web), C10–C20a)                                                                       |
| Server  | Node 22, TypeScript, Fastify, WebSocket, OpenTelemetry                                                  | ingest, alerts, reads, fan-out, decisions, semantic retention, tracing, the silence alarm ([apps/server](apps/server), C5–C20a)                                              |
| Infra   | Docker + compose; AWS CDK: ECR, ECS Fargate, ALB, S3 + CloudFront                                       | images, the compose stack and the k6 rig ([infra](infra), C19); the CDK stack ([infra/cdk](infra/cdk), C19) is synthesized and asserted and has never been deployed          |
...
SUDEP kills roughly 1 in 1,000 people with epilepsy per year, mostly unwitnessed during sleep ([CDC](https://www.cdc.gov/epilepsy/sudep/index.html), [Epilepsy Foundation](https://www.epilepsy.com/complications-risks/early-death-sudep)). Seizure-alert wearables are a real regulated product category, not a thought experiment: the Empatica Embrace cleared under product code POS with a decision date of 2018-01-26 (K172935), and the category itself is Class II under 21 CFR 882.1580. Both were read at their primary source on 2026-08-07 and are carried, with what could not be verified, in [docs/regulatory](docs/regulatory).
