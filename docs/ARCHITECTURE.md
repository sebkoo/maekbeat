...
| 7   | Dashboard fan-out      | WS push to apps/web         | Lambda fan-out                                                     | dev form shipped — C11 (apps/server/src/stream.ts, apps/web); per-subscriber send-buffer bound shipped — C19 (docs/DECISIONS.md #23); Lambda fan-out — C19                    |
| 8   | Caregiver notification | iOS notification            | same, triggered via fan-out                                        | dev form shipped — C16 (local notifications, apps/ios); an episode raised during a socket outage reaches it since C17; push — C19                                             |
