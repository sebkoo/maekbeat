...
| 2   | iOS gateway            | simulator transport in-app  | CoreBluetooth central, background streaming                        | shipped — C14 (reads the fan-out), C15 (central role + uplink to /ingest); the radio path is untested without hardware — apps/ios/README.md |
